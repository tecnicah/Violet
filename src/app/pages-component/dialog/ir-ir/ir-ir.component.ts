...
import { IrInspectionDetailComponent} from '../ir-inspection-detail/ir-inspection-detail.component';
import { IrRepairDetailComponent } from '../ir-repair-detail/ir-repair-detail.component';

@Component({
  selector: 'app-ir-ir',
  templateUrl: './ir-ir.component.html',
  styleUrls: ['./ir.component.scss']
})
export class IrIrComponent implements OnInit {
  ...
  public droppedFotosInspec(files: NgxFileDropEntry[], r, item) {
    ...
    for (const droppedFile of files) {
      ...
      if (droppedFile.fileEntry.isFile) {
        ...
        console.log("fotos inspection = =============================== " , item.photosInspecs)
      }
  ...
  deletePhoto_ins(id, index_pic, photosInspecs, index) {
    console.log("photosInspecs -------------------------------------------------" , photosInspecs)
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        if (id > 0) {
          this.delete_photo_section(id, index, index_pic);
        }
        else {
  ...
  delete_photo_section(id, index, index_pic) {

    this.loader.showLoader();
    this._services.service_general_put("HousingList/delete_photo_inspection", id).subscribe((response_bd => {
      this.loader.hideLoader();
      if (response_bd.success) {
        console.log("responde_bd delete photos inspection ==============================", response_bd);
        this.data_inspection[index].photosInspecs.splice(index_pic, 1);
  ...
  addAction(item){
   ...
   let edicion = this.data.edicion;
    ...
    const dialogRef = this._dialog.open(IrRepairDetailComponent, {
      data: {
