...
export class DialogWorkPermitComponent implements OnInit {
  ...
  getdata() {
    this._services.service_general_get("ImmigrationServices/GetWorkPermitById?id=" + this.data.service[0].id ).subscribe((data => {
      if (data.success) {
        console.log(data);
  ...
  getcity(visa) {
    this._services.service_general_get("Catalogue/GetState?country=" + visa.hostCountryId).subscribe((data => {
      if (data.success) {
        ...
        this.entryVisa = visa;
        this.request();
        if(this.entryVisa.commentsWorkPermits.length == 0){
          this.addReply();
...
public showDocumentDialogDetails( document:any, service_line:number = undefined ):void {
  ...
  removeReminder(i, id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        if (id == 0) {
          this.entryVisa.reminderWorkPermits.splice(i, 1);
        } else {
          this._services.service_general_delete("ImmigrationServices/DeleteReminderWP?id=" + id).subscribe((data => {
            if (data.success) {
  ...
  DeleteOnline(id) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this._services.service_general_delete("ImmigrationServices/DeleteDocumentWP?id=" + id).subscribe((data => {
          if (data.success) {
            const dialog = this._dialog.open(DialogGeneralMessageComponent, {
              data: {
                header: "Success",
  ...
  addDocument() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if(result.success){
        result.workPermitId = this.entryVisa.id;
  ...
  save() {
    ...
    let data_comment_aux = this.entryVisa.commentsWorkPermits;
    ...
    for(let i = 0; i < data_comment_aux.length; i++){
      if(data_comment_aux[i].reply != null && data_comment_aux[i].reply != undefined && data_comment_aux[i].reply.trim() != ''){
        ...
        this.entryVisa.commentsWorkPermits.push(data_comment_aux[i]);
      }
    }
    ...
    this._services.service_general_put("ImmigrationServices/PutWorkPermit", this.entryVisa).subscribe((data => {
      if (data.success) {
        console.log(data);
        const dialog = this._dialog.open(DialogGeneralMessageComponent, {
          data: {
