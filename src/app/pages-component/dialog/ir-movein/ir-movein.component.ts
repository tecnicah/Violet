...
@Component({
  selector: 'app-ir-movein',
  templateUrl: './ir-movein.component.html',
  styleUrls: ['./ir-movein.component.css']
})
export class IrMoveinComponent implements OnInit {
  ...
  GetInspRepBySection(key, servide_detail_id, section) {
    ...
    this._services.service_general_get("HousingList/GetInspRepBySection?key=" + key + "&servide_detail_id=" + servide_detail_id + "&section=" + section).subscribe((cd => {
      this.data_move_in = cd.result.value.propertyReports[0];

      this.dataSource = this.data_move_in.propertyReportSections;
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;

      console.log('ONLY Moe In GetInspRepBySection : ', cd, this.data_move_in);
